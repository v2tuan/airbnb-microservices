...
const steps: Array<{ key: StepKey; eyebrow: string; title: string; description: string }> = [
  {
    key: "place",
    eyebrow: "Step 1",
  ...
  },
  {
    ...
    description: "Add the details guests scan first: room type, guests, beds, and baths.",
  },
  {
    key: "location",
    eyebrow: "Step 3",
    title: "Where's your place located?",
    description: "Use a clear address and city so guests know the area before booking.",
  },
  {
    key: "photos",
    eyebrow: "Step 4",
  ...
  },
  {
    ...
    description: "You can update pricing, photos, and rules later from your host dashboard.",
  },
];
...
const propertyOptions: Array<{
  ...
  { value: "APARTMENT", label: "Apartment", description: "A rental unit in a shared building", icon: Building2 },
  { value: "HOUSE", label: "House", description: "A standalone home guests can settle into", icon: House },
  { value: "VILLA", label: "Villa", description: "A spacious, private stay with room to breathe", icon: Home },
  { value: "CONDO", label: "Condo", description: "A polished unit with building amenities", icon: Hotel },
  { value: "TOWNHOUSE", label: "Townhouse", description: "A multi-level home in a neighborhood", icon: Home },
  { value: "COTTAGE", label: "Cottage", description: "A cozy stay with a relaxed feel", icon: TentTree },
  { value: "BUNGALOW", label: "Bungalow", description: "A low-rise home with easy living", icon: Sparkles },
];

const roomOptions: Array<{ value: RoomType; label: string; description: string }> = [
  { value: "ENTIRE_PLACE", label: "An entire place", description: "Guests have the whole place to themselves." },
  { value: "PRIVATE_ROOM", label: "A private room", description: "Guests have a room and may share common spaces." },
  { value: "SHARED_ROOM", label: "A shared room", description: "Guests sleep in a shared area." },
];
...
export default function NewListingPage() {
  ...
  const selectedProperty = useMemo(
    () => propertyOptions.find((option) => option.value === form.propertyType) ?? propertyOptions[0],
    [form.propertyType]
  );

  const update = <K extends keyof ListingMutationPayload>(field: K, value: ListingMutationPayload[K]) => {
    setForm((current) => ({ ...current, [field]: value }));
  };

  const canContinue = () => {
    if (currentStep.key === "location") return !!form.address.trim() && !!form.city.trim() && !!form.country.trim();
    if (currentStep.key === "photos") return !!form.title.trim() && !!form.description.trim();
  ...
  if (!token) {
    return (
      <main className="mx-auto max-w-4xl px-6 pb-12">
        <div className="rounded-2xl border border-neutral-200 bg-white p-8">Please log in to create a listing.</div>
      </main>
    );
  }
    ...
    return (
      <main className="mx-auto max-w-4xl px-6 pb-12">
        <div className="rounded-2xl border border-neutral-200 bg-white p-8">
          <p className="text-xl font-semibold text-neutral-950">Only hosts can create listings</p>
          <p className="mt-2 text-neutral-500">Complete host onboarding before adding a new place.</p>
          <button
            type="button"
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-10 px-4 pb-12 sm:px-8 lg:grid-cols-[minmax(0,1fr)_390px]">
        <section className="pt-4 md:pt-10">
          ...
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-rose-500">{currentStep.eyebrow}</p>
            <h1 className="mt-3 text-4xl font-semibold tracking-tight text-neutral-950 md:text-5xl">
              {currentStep.title}
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-neutral-600">{currentStep.description}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-10 max-w-3xl">
                ...
                {propertyOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-5 text-lg font-semibold text-neutral-950">{option.label}</p>
            ...
            {currentStep.key === "basics" && (
              <div className="space-y-8">
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center justify-between rounded-2xl border p-5 text-left transition ${
                          active ? "border-neutral-950 bg-neutral-50" : "border-neutral-200 bg-white hover:border-neutral-950"
                        }`}
                      >
                        <span>
                          <span className="block text-lg font-semibold text-neutral-950">{option.label}</span>
                          <span className="mt-1 block text-sm text-neutral-500">{option.description}</span>
                        </span>
                        {active ? <Check className="size-5 text-neutral-950" /> : null}
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="rounded-2xl border border-neutral-200 px-5">
                  <Stepper label="Guests" value={form.maxGuests} min={1} icon={Users} onChange={(value) => update("maxGuests", value)} />
                  <Stepper label="Bedrooms" value={form.numBedrooms} min={0} icon={Home} onChange={(value) => update("numBedrooms", value)} />
                  <Stepper label="Beds" value={form.numBeds} min={1} icon={BedDouble} onChange={(value) => update("numBeds", value)} />
                  <Stepper
                    label="Bathrooms"
            ...
            {currentStep.key === "location" && (
              <div className="space-y-5">
                <label className="block">
                  <span className="text-sm font-semibold text-neutral-950">Street address</span>
                  <input
                    value={form.address}
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <label className="block">
                    <span className="text-sm font-semibold text-neutral-950">City</span>
                    <input
                      value={form.city}
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-neutral-950">Latitude</span>
                    <input
                      type="number"
                      step="0.000001"
                      value={form.latitude}
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-neutral-950">Longitude</span>
                    <input
                      type="number"
                      step="0.000001"
            ...
            {currentStep.key === "photos" && (
              <div className="space-y-6">
                <label className="block">
                  <span className="text-sm font-semibold text-neutral-950">Listing title</span>
                  <input
                    value={form.title}
                ...
                <div className="rounded-2xl border border-dashed border-neutral-300 p-6">
                  <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-base font-semibold text-neutral-950">Upload your cover photo</p>
                      <p className="mt-1 text-sm text-neutral-500">This uploads to your existing Cloudinary folder for listing photos.</p>
                    </div>
                  ...
                  {photoUrl ? (
                    <div className="mt-5 overflow-hidden rounded-2xl border border-neutral-200">
                      <Image src={photoUrl} alt="Cover preview" width={1100} height={720} className="aspect-[16/10] w-full object-cover" unoptimized />
                    </div>
                  ) : (
            ...
            {currentStep.key === "publish" && (
              <div className="space-y-6">
                <div className="rounded-2xl border border-neutral-200 p-6">
                  <p className="text-base font-semibold text-neutral-950">Nightly price</p>
                  <div className="mt-5 flex items-center gap-3">
                    <span className="text-5xl font-semibold text-neutral-950">$</span>
                    <input
                      type="number"
                      min="1"
                      value={basePrice}
                  ...
                  <p className="mt-4 text-sm text-neutral-500">Cleaning fee starts at 0 and service fee is set to 5%. You can change both after publishing.</p>
                </div>
                ...
                >
                  <span>
                    ...
                    <span className="mt-1 block text-sm text-neutral-500">Let guests book without waiting for approval.</span>
                  </span>
                  {form.instantBook ? <Check className="size-5 text-neutral-950" /> : null}
                </button>

                <div className="rounded-2xl bg-neutral-50 p-5">
                  <p className="text-sm font-semibold text-neutral-950">Ready to publish?</p>
                  <p className="mt-1 text-sm leading-6 text-neutral-500">
                    Your listing starts as a backend listing with pricing and an optional cover photo. Add more photos, rules, and availability from the edit page.
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h2 className="line-clamp-2 text-lg font-semibold text-neutral-950">
                      {form.title || `${selectedProperty.label} in ${form.city || "your city"}`}
                    </h2>
                    <p className="mt-1 text-sm text-neutral-500">
                      {form.city || "City"}, {form.country || "Country"}
                    </p>
                  </div>
                  <p className="shrink-0 text-sm font-semibold text-neutral-950">{formatPrice(basePrice, "USD")}</p>
                </div>
