...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleStripe(StripeException exception, HttpServletRequest request) {
        log.error("Stripe failure userId={} requestId={} path={} code={}",
                currentUserId(), requestId(request), request.getRequestURI(), exception.getCode(), exception);
        return build(HttpStatus.UNPROCESSABLE_ENTITY, "PAYMENT_PROVIDER_ERROR",
                "Payment provider could not process the request. Please try again.", request);
    }
